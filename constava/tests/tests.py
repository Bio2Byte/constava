...
class TestWrapper(unittest.TestCase):
    ...
    def test_0a_KDEModelFitting(self):
        ...
        cva = Constava(verbose=VERBOSE_LEVEL)
    ...
    def test_1a_KDEModelLoading(self):
        ...
        cva = Constava(verbose=VERBOSE_LEVEL, model_load=self.TEST_MODELDUMP0)
    ...
    def test_3a_KDEModelInference(self):
        """Test inference from pre-fitted KDE model"""
        logger.warning(f"TEST #{self.get_test_count()}: Inference from 'kde' conformational state model in JSON format...")
        input_files = glob.glob(f"{self.CONSTAVA_TEST_DATA_DIR}/xvg/ramaPhiPsi*.xvg")
        expected_result = f"{self.CONSTAVA_TEST_DATA_DIR}/xvg/result_kde.json"
        output_file  = tempfile.NamedTemporaryFile(prefix="kde.", suffix=".json", dir=self.TEST_TEMPDIR)
        cva = Constava(
            input_files = input_files,
            output_file = output_file.name,
            model_type = "kde",
        ...
        with open(os.path.join(self.TEST_TEMPDIR, "expected_test_3a_KDEModelInference.json"), "w", encoding="utf-8") as f:
            json.dump(expected_obj['results'], f)

        with open(output_file.name, "r", encoding="utf-8") as f_act:
            actual_obj = json.load(f_act)

        with open(os.path.join(self.TEST_TEMPDIR, "actual_test_3a_KDEModelInference.json"), "w", encoding="utf-8") as f:
    ...
    def test_1b_GridModelLoading(self):
        ...
        cva = Constava(verbose=VERBOSE_LEVEL, model_load=self.TEST_MODELDUMP1)
    ...
    def test_2b_GridModelInference(self):
        ...
        expected_result = f"{self.CONSTAVA_TEST_DATA_DIR}/csv/result_grid.csv"
        output_file = tempfile.NamedTemporaryFile(
    ...
    def test_3b_GridModelInference(self):
        """Test inference from pre-fitted grid-inference model"""
        logger.warning(f"TEST #{self.get_test_count()}: Inference from 'grid' conformational state model in JSON format...")
        input_files = f"{self.CONSTAVA_TEST_DATA_DIR}/csv/dihedrals.csv"
        expected_result = f"{self.CONSTAVA_TEST_DATA_DIR}/csv/result_grid.json"
        output_file  = tempfile.NamedTemporaryFile(prefix="grid.", suffix=".json", dir=self.TEST_TEMPDIR)
        c = Constava(
            input_files = input_files,
            output_file = output_file.name,
            model_type = "grid",
        ...
        with open(os.path.join(self.TEST_TEMPDIR, "expected_test_3b_GridModelInference.json"), "w", encoding="utf-8") as f:
            json.dump(expected_obj['results'], f)

        with open(output_file.name, "r", encoding="utf-8") as f_act:
            actual_obj = json.load(f_act)

        with open(os.path.join(self.TEST_TEMPDIR, "actual_test_3b_GridModelInference.json"), "w", encoding="utf-8") as f:
            json.dump(actual_obj['results'], f)
...
def run_unittest(verbose: int = 0):
    ...
    logger.warning(f"Starting Test Suite with verbosity={VERBOSE_LEVEL}")
