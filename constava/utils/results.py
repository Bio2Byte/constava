"""constava.utils.results contains classes that hold output information.

ConstavaResultsEntry stores state_propensities and state_variability for
...
class ConstavaResults:
    ...
    -----------
        method : str
            A string indicating the sub-sampling method with which the results
            were calculated
    ...
    def __init__(
        ...
        protein: ProteinEnsemble,
        state_labels: List[str],
        entries: List[ConstavaResultsEntry] = None,
    ):
        self.method = method
    ...
    def add_entry(
        self, new_entry: ConstavaResultsEntry, sorted_insertion: bool = False
    ):
        """Adds an additional entry to the result class"""

        if new_entry.residue.protein is not self.protein:
            raise EnsembleMismatchError(
                f"Result for residue {new_entry.residue} does not belong to the given Protein"
            )
