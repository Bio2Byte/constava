...
from ..calc.subsampling import (
    SubsamplingBootstrap,
    SubsamplingBootstrapSeries,
    SubsamplingWindow,
        ...
        if func.__name__ == "load_csmodel":
            ...
            new_cshash = hash(
                (
                    ...
                    args[2] if len(args) > 2 else kwargs.get("kde_bandwidth", 0.13),
                    args[3] if len(args) > 3 else kwargs.get("grid_points", 10_000),
                    (
                        args[4]
                        if len(args) > 4
                        else kwargs.get("model_data_degrees", False)
                    ),
                )
            )
        else:
            raise TypeError(
                "Decorator `cache_csmodel` only works with `load_csmodel` and `fit_csmodel`"
            )
...
class Constava:
    """
    Interface class for all functionalities of Constava. This tool allows the
    probabilistic description of protein conformation from a set of density-defined
    conformational states. Six solution NMR-defined conformational states
        are provided by default, but users can define their own probability density
    functions with the method fit_csmodel().
    ...
    --------
		...
		initialize_reader(file_format="auto", in_degrees=False):
            Initializes an EnsembleReader for reading input files, with the format specified by 'file_format' and whether
            the data is in degrees specified by 'in_degrees'.
        initialize_writer(outfile, file_format="auto", float_precision=4, indent_size=0):
    ...
    def run(self) -> None:
        ...
        reader = self.initialize_reader(
            file_format=self.get_param("input_format"),
        ...
        writer = self.initialize_writer(
            ...
            file_format=self.get_param("output_format"),
    ...
    def initialize_reader(
        self, file_format: str = "auto", in_degrees: bool = False
    ) -> EnsembleReader:
    ...
    def initialize_calculator(
        self,
        csmodel: ConfStateModelABC = None,
        window: List[int] = None,
        window_series: List[int] = None,
        bootstrap: List[int] = None,
        bootstrap_series: List[int] = None,
        ...
        for sample_size in bootstrap or []:
            new_method = SubsamplingBootstrap(
                sample_size, bootstrap_samples, seed=bootstrap_seed
            )
            logger.info(f"... adding subsampling method: {new_method.getShortName()}")
            calculator.add_method(new_method)
        ...
        for sample_size in bootstrap_series or []:
            new_method = SubsamplingBootstrapSeries(
                sample_size, bootstrap_samples, seed=bootstrap_seed
            )
            logger.info(f"... adding subsampling method: {new_method.getShortName()}")
            calculator.add_method(new_method)
