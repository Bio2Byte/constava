...
class ConStaVa:
	...
	def read_input_files(self, infile=None, degrees=False, input_format='auto'):
		...
		reader = EnsembleReader(filetype_str=self.input_format,
		                        degrees2radians=self.degrees)
		self.ensemble = reader.readFiles(*self.infile)
	...
	def save_kde(self):
		self.pdfestimator.dump_pickle(self.dump_kde)
